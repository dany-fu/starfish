...
class TileCoordinate:
    """Data class for encapsulating the location of a tile in a"""
    fov_id: int
    round_id: int
    ch_id: int
    zplane_id: int
...
def build_irregular_image(
        tile_coordinates: Iterable[TileCoordinate],
    ...
    """
    def reducer_to_sets(
            accum: Sequence[MutableSet[int]], update: TileCoordinate,
        ...
        for accum_elem, update_elem in zip(accum, astuple(update)):
            accum_elem.add(update_elem)
            result.append(accum_elem)
        return result
...
def build_image(
    ...
    tile_coordinates = [
        TileCoordinate(fov_id, selector[Axes.ROUND], selector[Axes.CH], selector[Axes.ZPLANE])
        for fov_id in fovs
...
def write_irregular_experiment_json(
        ...
        *,
        image_tile_coordinates: Mapping[str, Iterable[TileCoordinate]],
...
def write_experiment_json(
    ...
    image_tile_coordinates: MutableMapping[str, Iterable[TileCoordinate]] = dict()
